     ...
     <Button type="primary" onClick={() => setIsModalVisible(true)}>
      ...
      >
        <Form
          ...
          wrapperCol={{ span: 18 }}
        >
          <Form.Item label="食物名称" name="foodName" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item label="食物重量" name="foodAmount" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item label="份额" name="servings" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item label="进餐时间" name="eatingTime" rules={[{ required: true }]}>
            <DatePicker showTime format="YYYY-MM-DD HH:mm" />
          </Form.Item>
          <Form.Item label="烹饪方法" name="cookingMethod" rules={[{ required: true }]}>
            <Select>
